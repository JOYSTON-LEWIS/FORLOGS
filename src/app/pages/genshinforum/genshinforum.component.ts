...
import { GenshinforumService } from 'src/app/services/genshinforum.service';

@Component({
  selector: 'app-genshinforum',
  templateUrl: './genshinforum.component.html',
  styleUrls: ['./genshinforum.component.css']
})
export class GenshinforumComponent implements OnInit {
  giforum: any;
  forumtitle: string;
  forumcontent: string;
  uid: string;
  ...
  constructor(public authservice: AuthService, private router: Router,
              public db: AngularFireDatabase, private service: GenshinforumService, private firest: AngularFirestore) {
              }

   ngOnInit() {
    this.service.read_Post().subscribe(data => {

      this.giforum = data.map(e => {
        return {
          id: e.payload.doc.id,
          isEdit: false,
